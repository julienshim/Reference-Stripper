...
In the `webpack.config.js` file, the `output` and `devServer` are currently pointing to the `build` folder as required by [gh-pages](https://yarnpkg.com/package/gh-pages), but can be pointed to a different folder along with static files.
